...
import 'package:letmegoo/core/utils/logger.dart'; // 🔥 ADD THIS IMPORT
...
class DeviceService {
  ...
  static Future<String> _getPushNotificationStatus() async {
    try {
      ...
      AppLogger.debug('Error checking push notification status: $e');
  ...
  static Future<void> updateDeviceToken(String newToken) async {
    try {
      ...
      AppLogger.debug('Device token update response: ${response.statusCode}');
  ...
  static Future<void> unregisterDevice() async {
    try {
      ...
      AppLogger.debug('Device unregister response: ${response.statusCode}');
      ...
      AppLogger.debug('Error unregistering device: $e');
  ...
  static Future<Map<String, dynamic>?> checkDeviceRegistration() async {
    try {
      ...
      AppLogger.debug('Checking device registration with token: $fcmToken');
      ...
      AppLogger.debug('Device check response: ${response.statusCode}');
      ...
      AppLogger.debug('Device check error: $e');
  ...
  static Future<bool> ensureDeviceRegistered() async {
    try {
      ...
      if (deviceData != null) {
        AppLogger.debug('Device already registered: ${deviceData['id']}');
      ...
      AppLogger.debug('Device not registered, registering now...');

      // Device not registered, register it
      final registrationResult = await registerDevice();

      if (registrationResult != null) {
        AppLogger.debug('Device registered successfully: ${registrationResult['id'] ?? 'unknown'}',);
      ...
      AppLogger.debug('Error ensuring device registration: $e');
