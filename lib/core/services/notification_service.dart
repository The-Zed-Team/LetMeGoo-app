...
class NotificationService {
  ...
  static Future<void> initialize() async {
    try {
      ...
      if (settings.authorizationStatus == AuthorizationStatus.authorized ||
        ...
        if (token != null) {
          AppLogger.debug('Initial FCM token retrieved: ${token.substring(0, 10)}...');
          _pendingToken = token;
        }
        ...
        FirebaseAuth.instance.authStateChanges().listen((User? user) {
          if (user != null && _pendingToken != null) {
            AppLogger.debug('User authenticated, updating pending token');
  ...
  static void _handleTokenRefresh(String newToken) async {
    ...
    if (user != null) {
      ...
      AppLogger.debug('Token refresh received but user not authenticated, storing for later',);
    }
  }
  ...
  static Future<void> _checkInitialMessage() async {
    ...
    if (initialMessage != null) {
      AppLogger.debug('App opened from notification: ${initialMessage.notification?.title}',);
    ...
    if (message.data.isNotEmpty) {
      ...
      switch (type) {
        case 'report_status':
          ...
          AppLogger.debug('Report notification: $reportId');
          break;
        case 'vehicle_reported':
          // Could navigate to specific vehicle report
          AppLogger.debug('Vehicle reported: $reportId');
          break;
        default:
  ...
  static void _navigateToHomePage() {
    AppLogger.debug('🏠 Navigating to homepage...');
    ...
    if (_navigatorKey?.currentContext != null) {
          ...
          builder:
              (context) => HomePage(
                onNavigate: (index) {
                  ...
                  AppLogger.debug('Homepage navigation: $index');
                },
                onAddPressed: () {
                  // Handle add button if needed
                  AppLogger.debug('Homepage add pressed');
                },
              ),
        ),
      ...
      AppLogger.debug('✅ Navigation to homepage completed');
    } else {
      AppLogger.debug('❌ Navigator context not available');
  ...
  static void _navigateToMainApp() {
    AppLogger.debug('🔄 Fallback: Navigating to MainApp...');
    ...
    if (_navigatorKey?.currentContext != null) {
      ...
      AppLogger.debug('✅ Navigation to MainApp completed');
    }
  }
