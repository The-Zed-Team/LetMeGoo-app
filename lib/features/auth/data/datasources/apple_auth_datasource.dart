...
class AppleAuthService {
  ...
  static Future<UserCredential?> signInWithApple() async {
    try {
      ...
      if (kDebugMode) {
        AppLogger.debug('Starting Apple authentication flow');
      }
      ...
      if (kDebugMode) {
        AppLogger.debug('Successfully authenticated with Apple');
        AppLogger.debug('Apple User ID: ${appleCredential.userIdentifier ?? "Not provided"}',);
        AppLogger.debug('Email: ${appleCredential.email ?? "Not provided"}');
        AppLogger.debug('Name: ${appleCredential.givenName ?? ""} ${appleCredential.familyName ?? ""}',);
      }
      ...
      if (kDebugMode) {
        AppLogger.debug('Using email: $email for Firebase authentication');
      }
      ...
      try {
        ...
        if (kDebugMode) {
          AppLogger.debug('Successfully signed in existing user with email/password');
        }
        ...
        if (kDebugMode) {
          AppLogger.debug('Sign-in failed, creating new user: $signInError');
        }
        ...
        if (user != null) {
          ...
          if (displayName.isNotEmpty) {
            ...
            if (kDebugMode) {
              AppLogger.debug('Updated user display name to: $displayName');
            }
          }
        }
        ...
        if (kDebugMode) {
          AppLogger.debug('Successfully created new user with email/password');
        }
      ...
      if (e.code == AuthorizationErrorCode.canceled) {
        if (kDebugMode) {
          AppLogger.debug('User canceled Apple Sign In');
        }
        return null;
      }

      if (kDebugMode) {
        AppLogger.debug('Apple authorization error: ${e.code} - ${e.message}');
      }
      return null;
    } catch (e) {
      if (kDebugMode) {
        AppLogger.debug('Error during Apple Sign In: $e');
      }
      return null;
    }
  }
