...
class GoogleAuthService {
  ...
  static Future<UserCredential?> signInWithGoogle() async {
    try {
      ...
      if (googleUser == null) {
        if (kDebugMode) {
          AppLogger.debug(
            'Google Sign In: User cancelled the sign-in process.',
          );
        }
        return null;
      }
      ...
      if (kDebugMode) {
        AppLogger.debug(
          'Google Sign In: User signed in -> ${googleUser.email}',
        );
      }
      ...
      if (kDebugMode) {
        AppLogger.debug('Google Auth: Obtained access and ID tokens.');
      }
      ...
      if (kDebugMode) {
        AppLogger.debug('Google Auth: Created Firebase credential.');
      }
      ...
      try {
        ...
        if (kDebugMode) {
          AppLogger.debug('Device registration failed: $e');
        }
      ...
      if (kDebugMode) {
        AppLogger.debug('Firebase Auth Error: [${e.code}] ${e.message}');
      }
      return null;
    } catch (e) {
      if (kDebugMode) {
  ...
  static Future<void> signOut() async {
    try {
      if (kDebugMode) {
        AppLogger.debug('🚪 Starting sign out process...');
      }
      ...
      try {
        ...
        if (kDebugMode) {
          AppLogger.debug('📡 Logout API call handled by AuthService');
        }
      } catch (e) {
        if (kDebugMode) {
          AppLogger.debug('⚠️ Logout API call skipped or failed: $e');
        }
      }
      ...
      try {
        ...
        if (kDebugMode) {
          AppLogger.debug('⚠️ Device unregister failed: $e');
        }
      ...
      if (kDebugMode) {
        AppLogger.debug('✅ Sign out successful from Firebase and Google');
      }
    } catch (e) {
      if (kDebugMode) {
        AppLogger.debug('❌ Error during sign out: $e');
      }
      ...
      try {
        await _auth.signOut();
        if (kDebugMode) {
          AppLogger.debug('✅ Firebase sign out successful (fallback)');
        }
      } catch (firebaseError) {
        if (kDebugMode) {
          AppLogger.debug('❌ Firebase sign out failed: $firebaseError');
        }
      }
      ...
      try {
        ...
        if (kDebugMode) {
          AppLogger.debug('✅ Google sign out successful (fallback)');
        }
      } catch (googleError) {
        if (kDebugMode) {
          AppLogger.debug('❌ Google sign out failed: $googleError');
        }
      }
  ...
  static Future<void> signOutWithAPI() async {
    try {
      if (kDebugMode) {
        AppLogger.debug('🚪 Starting complete sign out process with API...');
      }
      ...
      if (kDebugMode) {
        AppLogger.debug('✅ Complete sign out successful');
      }
    } catch (e) {
      if (kDebugMode) {
        AppLogger.debug('❌ Error during complete sign out: $e');
      }
      ...
      try {
        await Future.wait([_auth.signOut(), _googleSignIn.signOut()]);
        if (kDebugMode) {
          AppLogger.debug('✅ Local sign out successful (fallback)');
        }
      } catch (localError) {
        if (kDebugMode) {
          AppLogger.debug('❌ Local sign out also failed: $localError');
        }
        throw localError;
      }
    }
  ...
  static Future<void> _callLogoutAPI() async {
    ...
    try {
      ...
      if (firebaseUser == null) {
        if (kDebugMode) {
          AppLogger.debug('No Firebase user found, skipping logout API call');
        }
        return;
      }
      ...
      if (idToken == null) {
        if (kDebugMode) {
          AppLogger.debug('Failed to get ID token, skipping logout API call');
        }
        return;
      }
      ...
      if (deviceId == null) {
        if (kDebugMode) {
          AppLogger.debug('Failed to get device ID, skipping logout API call');
        }
        return;
      }
      ...
      if (kDebugMode) {
        AppLogger.debug('📤 Logout API response: ${response.statusCode}');
      }
      ...
      if (kDebugMode) {
        AppLogger.debug('❌ Logout API error: $e');
      }
