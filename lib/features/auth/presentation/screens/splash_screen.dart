...
class _SplashScreenState extends State<SplashScreen>
  ...
  Future<void> _checkAuthenticationStatus() async {
    try {
      ...
      if (status == 'profile_completed') {
        ...
        AppLogger.debug('Unknown user status: $status');
        _updateLoadingText('Something went wrong...');
        await Future.delayed(const Duration(milliseconds: 1000));
        _navigateToLogin();
      }
    } catch (e) {
      AppLogger.debug('Authentication check error: $e');
  ...
  Future<void> _ensureDeviceRegistration() async {
    try {
      ...
      if (isRegistered) {
        AppLogger.debug('Device registration confirmed');
      } else {
        AppLogger.debug('Device registration failed, but continuing...');
      }
    } catch (e) {
      AppLogger.debug('Device registration error: $e');
