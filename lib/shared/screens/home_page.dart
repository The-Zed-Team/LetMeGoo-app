...
import 'package:letmegoo/features/report/presentation/providers/report_provider.dart';
import 'package:letmegoo/features/report/presentation/providers/report_cache_provider.dart';
import 'package:letmegoo/features/report/presentation/widgets/buildreportsection.dart';
import 'package:letmegoo/core/widgets/builddivider.dart';
...
class _HomePageState extends ConsumerState<HomePage>
  ...
  Future<void> _loadReportsIfNeeded() async {
    try {
      ...
      AppLogger.debug('🔍 Loading reports check:');
      AppLogger.debug('  - Cache valid: ${cache.isCacheValid}');
      AppLogger.debug('  - Total reports: ${currentState.totalReports}');
      AppLogger.debug('  - Is loading: ${currentState.isLoading}');
      AppLogger.debug('  - Has error: ${currentState.error != null}');

      if (!cache.isCacheValid || currentState.totalReports == 0) {
        AppLogger.debug('🔥 Starting to load reports...');
        reportsNotifier
            .loadReports()
            .then((_) {
              if (mounted) {
                ...
                AppLogger.debug('✅ Reports loaded successfully:');
                AppLogger.debug(
                  '  - Live by user: ${newState.liveByUser.length}',
                );
                AppLogger.debug(
                  '  - Live against user: ${newState.liveAgainstUser.length}',
                );
                AppLogger.debug(
                  '  - Solved by user: ${newState.solvedByUser.length}',
                );
                AppLogger.debug(
            ...
            .catchError((error) {
              AppLogger.debug('❌ Error loading reports: $error');
              if (mounted) {
                _showErrorSnackBar('Failed to load reports. Please try again.');
              }
            });
      } else {
        AppLogger.debug('✅ Using cached reports');
      }
    } catch (e) {
      AppLogger.debug('❌ Exception in _loadReportsIfNeeded: $e');
      if (mounted) {
  ...
  Future<void> _onRefresh() async {
    ...
    try {
      ...
      if (mounted) {
        cache.updateCacheTime();
        final newState = ref.read(reportsProvider);
        AppLogger.debug('✅ Reports refreshed:');
        AppLogger.debug('  - Total: ${newState.totalReports}');
        _showSuccessSnackBar('Reports refreshed successfully');
      }
    } catch (e) {
      AppLogger.debug('❌ Error refreshing: $e');
      if (mounted) {
            ...
            Expanded(
              child: Consumer(
                builder: (context, ref, child) {
                  ...
                  AppLogger.debug('🎨 Building UI with state:');
                  AppLogger.debug('  - Loading: ${reportsState.isLoading}');
                  AppLogger.debug(
                    '  - Has no reports: ${reportsState.hasNoReports}',
                  );
                  AppLogger.debug('  - Error: ${reportsState.error}');

                  if (reportsState.isLoading) {
                    AppLogger.debug('🔄 Showing loading widget');
                    return _LoadingWidget(screenHeight: screenHeight);
                  }

                  if (reportsState.error != null && reportsState.hasNoReports) {
                    AppLogger.debug('❌ Showing error widget');
                    return _ErrorWidget(
                  ...
                  if (reportsState.hasNoReports) {
                    AppLogger.debug('🔭 Showing empty widget');
                    return _EmptyWidget(
                      screenHeight: screenHeight,
                  ...
                  AppLogger.debug('📊 Showing reports content');
                  ...
                  try {
                    ...
                    AppLogger.debug('  - Error in liveByUserFormatted: $e');
                    liveByUserFormatted = [];
                  }
                  ...
                  try {
                    ...
                    AppLogger.debug(
                      '  - Error in liveAgainstUserFormatted: $e',
                    );
                  ...
                  try {
                    ...
                    AppLogger.debug('  - Error in solvedByUserFormatted: $e');
                  ...
                  try {
                    ...
                    AppLogger.debug(
                      '  - Error in solvedAgainstUserFormatted: $e',
                    );
